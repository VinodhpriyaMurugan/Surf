...
export default function Registeremployee() {
  ...
  useEffect(() => {
    ...
    userservices.getDivison(token).then((Response) => {
      for (var i = 0; i < Response.data.length; i++) {
        divisionList[i] = {
          value: Response.data[i].division_name,
          label: Response.data[i].division_name,
        };
      }
  ...
  const loadBranch = (value) => {
    ...
    userservices.getBranch(value.label, token).then((Response) => {
      for (var i = 0; i < Response.data.length; i++) {
        branchList[i] = {
          value: Response.data[i].branch_name,
          label: Response.data[i].branch_name,
        };
      }
  ...
  const save = (e) => {
    ...
    } else if(roleDisplay !="" && roleDisplay !=null) {
      roles = [roleUser,roleDisplay];
    }
    let employee = {
  ...
  return (
    <div className="updateDiv">
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
        ...
        <button className="hi" disabled={!file} onClick={uploadFileData}>
          Upload
        </button>
      </div>
      <form>
        ...
        <div className="form-group">
          ...
          <input
            ...
            name="email"
            id="emailid"
            pattern="^[a-zA-Z0-9+_.-]+@tpfsoftware.com"
            value={email}
            onChange={(e) => {
        ...
        <td>
          ...
          <input
            ...
            value="ROLE_ADMIN"
            onClick={(e) => {
              setRoleDisplay(e.target.value);
            }}
          />
           <b>&nbsp;Display&emsp; </b>
        </td>
        <button className="btn-submitadduser" onClick={save}>
          Save
        </button>
      </form>
    </div>
